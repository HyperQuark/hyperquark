<template>
  ...
  <template v-else>
    <Suspense v-if="file.json !== null">
      <ProjectPlayer :json="file.json" :assets="file.assets" :title="file.title"
        :author="file.author" :zip="file.zip"></ProjectPlayer>
      <template #fallback>
        <Loading>Loading...</Loading>
      </template>
    </Suspense>
    <div v-else>Upload a project: <ProjectFileInput @error="err"></ProjectFileInput>
    </div>
  </template>
</template>
