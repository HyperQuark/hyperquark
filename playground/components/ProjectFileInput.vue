<template>
  <Loading v-if="loading"></Loading><input v-else type="file" ref="fileInput" @input="handleFileInput">
</template>
...
<script setup>
  ...
  async function handleFileInput() {
    ...
    try {
      ...
      console.log('error', e)
      emit('error', e.hasOwnProperty('validationError') ? JSON.stringify(e) : e.toString() + (e.stack ? '\n' + e.stack : ''));
      loading.value = false;
      return;
    }
